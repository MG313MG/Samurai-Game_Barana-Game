...
public enum IEnumerator_Timer_for_Samurai {attak_timer, can_attak_by_sword_timer, can_attak_by_bow_timer, timer_to_minus_1_from_jump, timer_for_false_ishitting, timer_for_go_back};
...
public class SamuraiPlayer : MonoBehaviour {
    ...
    {
		//idle
		if (isGrounded && !Input.GetKey(KeyCode.D) && !Input.GetKey(KeyCode.A) && !isHitting)
		{
			Samurai_Mode = Samurai_Modes.idle;
		}
		//Walk
        if (isGrounded && (Input.GetKey(KeyCode.D) || Input.GetKey(KeyCode.A)) && !isRunning && !isAttaking_byBow && !isJumping && !isHitting)
        {
            Samurai_Mode = Samurai_Modes.walk;
        }
		//Run
		if (isGrounded && (Input.GetKey(KeyCode.D) || Input.GetKey(KeyCode.A)) && Input.GetKey(KeyCode.LeftControl) && Stamina != 0 && !isAttakingbySword && !isAttaking_byBow && !isJumping && !isHitting)
		{
			Samurai_Mode = Samurai_Modes.run;
		}
		//Jump_In Sky_Fall
		if (Input.GetKeyDown(KeyCode.Space) && Can_Jumping > 0 && !isAttakingbySword && !isAttaking_byBow && !isHitting) 
		{
		...
		if (rb.linearVelocity.y == 0 && isGrounded && isJumping)
		...
		if (Input.GetMouseButtonDown(0) && !isJumping && !isRunning && isCanAttakingbySword && !isAttaking_byBow && !isHitting)
			Samurai_Mode = Samurai_Modes.attak_by_sword;
		//Attak by Bow
		if (Input.GetMouseButton(1) && !isJumping && !isRunning && !isAttakingbySword && isCanAttakingbyBow && !isHitting)
		{
